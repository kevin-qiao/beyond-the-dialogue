...
- **技能与 MCP 管理（已接入）** — 在设置里手动粘贴标准 `mcp.json` 形状添加 MCP 服务器（应用也会把配置同步写入自己数据目录下的 `mcp.json`，仅作为可查看的副本，运行时不读它）；获授权类型（按类型授权）的交互会话会经由 `pi-mcp-adapter` 真正连到外部系统。受限后台任务在任何配置下都不获得授权，所以这条连接只对显式授权的交互会话生效；应用目前尚未提供这样的界面会话（聊天仍是单轮），下一步补上。
...
- **活的连接器（已落地）** — `add-mcp-support` 已接通获授权工具服务器到交互会话的连接（经由 `pi-mcp-adapter@2.35.0`）。仍待补上的是：确认条背后逐次变更的远端*执行*（FR-022 第二次修订，需要模型外的工具调用），以及第一个面向用户的界面会话表层——让获授权的服务器能从界面里被够到，而不只是停在接缝处。
...
- Linux 打包版依赖 `libgtk-3-0`、`libnss3`、`libasound2`，以及 `libsecret-1-0`（仅当用户添加的 MCP 服务器主动使用系统凭据存储时才会触达，应用自身从不调用）。
