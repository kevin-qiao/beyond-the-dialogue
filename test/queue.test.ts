...
test('4.2 restart re-queues interrupted running jobs', async () => {
  ...
  db.db
    ...
    .run('job-interrupted', 'preprocess', null, 'running', 1, new Date().toISOString())
  const running = listRunningJobs(db.db)
  assert.equal(running.length, 1)
  const requeued = new JobQueue(db.db, 2)
  let ran = 0
  requeued.register('preprocess', async () => {
    ran++
  })
...
test('4.4 transient errors are retried; permanent errors fail immediately', async () => {
  ...
  const job = q.enqueue('preprocess', null)
  await sleep(120)
...
test('4.7 suggestion and analysis job kinds dispatch correctly', async () => {
  ...
  q.register('preprocess', async () => {
    kinds.push('preprocess')
  })
  q.enqueue('suggestion', 'task-1')
  q.enqueue('preprocess', 'task-2')
  await sleep(80)
  assert.deepEqual(kinds.sort(), ['preprocess', 'suggestion'])
  db.close()
})
...
test('6.4 cancel: running job is failed with "cancelled by user" and onCancel fires', async () => {
  ...
  q.register('preprocess', async (ctx) => {
    ctx.onCancel(() => {
      onCancelFired = true
    })
    await gate
...
test('6.4 cancel: queued job is failed in place and never runs', async () => {
  ...
  const first = q.enqueue('preprocess', null)
  const second = q.enqueue('preprocess', null)
  await sleep(30)
  q.cancel(second.id)
  release()
  await sleep(80)
